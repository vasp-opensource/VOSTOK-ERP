-- batch_assembly_batches: батч сборочных партий с отдельным логом assembly_batches_log.
-- Интервал запуска: каждые 47 секунд.

CREATE TABLE IF NOT EXISTS `assembly_batches_log` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_assembly_batches_log_run` (`run_id`),
    KEY `idx_assembly_batches_log_created_at` (`created_at`),
    KEY `idx_assembly_batches_log_procedure` (`procedure_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP EVENT IF EXISTS ev_batch_assembly_batches_47s;

DROP PROCEDURE IF EXISTS batch_assembly_batches;

DELIMITER $$

CREATE PROCEDURE batch_assembly_batches()
BEGIN
    ...
    BEGIN
        ...
        IF v_batch_lock = 1 THEN
            DO RELEASE_LOCK('batch_assembly_batches');
        END IF;
    ...
    SELECT GET_LOCK('batch_assembly_batches', 0) INTO v_batch_lock;
    ...
    ELSE
        ...
        VALUES (
            ...
            ROUND(TIMESTAMPDIFF(MICROSECOND, v_batch_started, v_batch_finished) / 1000, 3),
            'BLOCKED',
            'Blocked: batch_assembly_batches lock is already held',
            CURRENT_TIMESTAMP(6)
        );
    END IF;
END$$
...
CREATE EVENT ev_batch_assembly_batches_47s
ON SCHEDULE EVERY 47 SECOND
ON COMPLETION PRESERVE
ENABLE
COMMENT 'ERP: assembly batches batch every 47 seconds'
DO CALL batch_assembly_batches();
