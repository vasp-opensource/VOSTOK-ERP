...
BEGIN
    ...
    IF v_lock_ok = 1 THEN
        ...
        /* Защита от ложного неттинга в одном такте:
           ...
           Важно: откладываем до любых UPDATE Transactions, иначе строка может получить
           Status_warehouse='В закупке' без соответствующей дельты в Main.inProcess_purchase. */
