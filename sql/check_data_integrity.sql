...
-- Проверки, связанные с таблицей Import, выполняет самостоятельная процедура check_imported_quantity.
...
--   только в «В ожидании», со складом «Дефицит закупки», либо уже «Заменено»/«Отменено» (архив после unite и т.п.).
--
-- Дополнительные идеи проверок (при необходимости включить отдельно):
--   • (идея) каждый id в цепочке linked_transaction (через "; ") существует в Transactions;
