-- deficit: построчная обработка move (внешний -> брак/отгрузка/изделие)
-- со Status_transaction='В ожидании' и Status_warehouse='Новая'.
-- Логика:
-- 1) Родительская строка всегда закрывается как "Заменено" (linked_transaction = свой id).
-- 2) Создаются дочерние move-строки (со ссылкой на родителя):
--    - часть со склада -> Status_warehouse='Норма', Order_wh='В комплектации';
--    - часть, покрытая ожидаемыми поступлениями -> Status_warehouse='Норма', Order_purch='Дефицит закупки';
--    - дефицитная часть -> Status_warehouse='Дефицит закупки', Order_purch='Дефицит закупки'.
-- 3) Для дефицитной части создается change: внешний -> склад, Quantity_change = дефицит.
-- 4) Приоритет обработки move: брак, отгрузка, изделие.
-- Блокировка: lock_process_move_deficit_wh_to_shop
...
BEGIN
    ...
    DECLARE v_part_expected BIGINT DEFAULT 0;
    DECLARE v_part_deficit BIGINT DEFAULT 0;
    ...
    IF v_lock_ok = 1 THEN
        ...
        DROP TEMPORARY TABLE IF EXISTS tmp_deficit_expected_used;
        CREATE TEMPORARY TABLE tmp_deficit_expected_used (
        ...
        read_loop: LOOP
            ...
            SELECT
                ...
                COALESCE(m.inProcess_purchase, 0),
                COALESCE(m.inProcess_manufacturing, 0)
            INTO
                v_stock,
                v_expect_purch,
                v_expect_prod
            ...
            SET v_stock = COALESCE(v_stock, 0);
            SET v_expect_purch = COALESCE(v_expect_purch, 0);
            SET v_expect_prod = COALESCE(v_expect_prod, 0);
            ...
            WHERE t.ERP_ID = v_erp_id
              ...
              AND t.where_to IN ('брак', 'отгрузка', 'изделие')
              AND t.Status_transaction = 'В ожидании'
              AND t.Status_warehouse IN ('Новая', 'Ожидание поставки', 'Ожидание поступления');
            ...
            SELECT COALESCE(u.used_qty, 0)
              INTO v_expected_used
            FROM tmp_deficit_expected_used u
            ...
            UPDATE `Transactions`
               SET Status_transaction = 'Заменено',
                   ...
                   Status_warehouse   = CASE
                                            WHEN v_total_avail = 0 THEN 'Дефицит закупки'
                                            ELSE 'Норма'
                                        END,
                   updated_by         = 'deficit',
            ...
            IF v_req_qty > 0 THEN
                SET v_part_stock = LEAST(GREATEST(v_stock, 0), v_req_qty);
                SET v_rest = v_req_qty - v_part_stock;

                SET v_part_expected = LEAST(v_expected_avail, v_rest);
                SET v_rest = v_rest - v_part_expected;

                SET v_part_deficit = v_rest;
                ...
                IF v_part_stock > 0 THEN
                    ...
                    SELECT
                        ...
                        'Норма', t.Document_no, t.Zakaz_no, t.Date_needed, t.Date_expected, t.Cost_total_rub
                    FROM `Transactions` t
                    WHERE t.id = v_tx_id;
                ...
                IF v_part_expected > 0 THEN
                    ...
                    SELECT
                        t.ERP_ID, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'deficit', 'deficit', v_tx_id,
                        'move', t.where_from, t.where_to, v_part_expected, 0, 'В ожидании',
                        t.Project, t.Target_assembly, t.Supplied_component_number, t.Component_revision, t.Component_name,
                        t.Quantity_in_target_assembly, t.Quantity_of_target_assemblies, t.Components_quantity_in_assembly, t.Component_type,
                        t.For_supplied_as_assembly_components_provided_by_supplier, t.Part_material, t.Producer, t.Catalogue_number,
                        t.Producer_article, t.Distributer, t.Distributer_article, t.MBOM_type, t.Mass_kg, t.Unit_of_measure,
                        t.Height, t.Width, t.Length, t.Advanced_group, t.Address, 'Дефицит закупки', t.Order_wh, t.Order_prod, t.Order_OTK,
                        'Норма', t.Document_no, t.Zakaz_no, t.Date_needed, t.Date_expected, t.Cost_total_rub
                    FROM `Transactions` t
                    WHERE t.id = v_tx_id;

                    INSERT INTO tmp_deficit_expected_used (erp_id, used_qty)
                    VALUES (v_erp_id, v_part_expected)
                    ON DUPLICATE KEY UPDATE used_qty = used_qty + VALUES(used_qty);
                END IF;

                IF v_part_deficit > 0 THEN
                    INSERT INTO `Transactions` (
                        ERP_ID, created_at, updated_at, created_by, updated_by, linked_transaction,
                        type, where_from, where_to, Quantity_of_parts_total, Quantity_change, Status_transaction,
                        Project, Target_assembly, Supplied_component_number, Component_revision, Component_name,
                    ...
                    SELECT
                        t.ERP_ID, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'deficit', 'deficit', v_tx_id,
                        'move', t.where_from, t.where_to, v_part_deficit, 0, 'В ожидании',
                        t.Project, t.Target_assembly, t.Supplied_component_number, t.Component_revision, t.Component_name,
                        t.Quantity_in_target_assembly, t.Quantity_of_target_assemblies, t.Components_quantity_in_assembly, t.Component_type,
                        t.For_supplied_as_assembly_components_provided_by_supplier, t.Part_material, t.Producer, t.Catalogue_number,
                        t.Producer_article, t.Distributer, t.Distributer_article, t.MBOM_type, t.Mass_kg, t.Unit_of_measure,
                        t.Height, t.Width, t.Length, t.Advanced_group, t.Address, 'Дефицит закупки', t.Order_wh, t.Order_prod, t.Order_OTK,
                        'Дефицит закупки', t.Document_no, t.Zakaz_no, t.Date_needed, t.Date_expected, t.Cost_total_rub
                    FROM `Transactions` t
                    WHERE t.id = v_tx_id;
                    ...
                    SELECT
                        t.ERP_ID, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'deficit', 'deficit', v_tx_id,
                        'change', 'внешний', 'склад', 0, v_part_deficit, 'В ожидании',
                        t.Project, t.Target_assembly, t.Supplied_component_number, t.Component_revision, t.Component_name,
                        t.Quantity_in_target_assembly, t.Quantity_of_target_assemblies, t.Components_quantity_in_assembly, t.Component_type,
                        t.For_supplied_as_assembly_components_provided_by_supplier, t.Part_material, t.Producer, t.Catalogue_number,
                        t.Producer_article, t.Distributer, t.Distributer_article, t.MBOM_type, t.Mass_kg, t.Unit_of_measure,
                        t.Height, t.Width, t.Length, t.Advanced_group, t.Address, 'Ожидание закупки', t.Order_wh, t.Order_prod, t.Order_OTK,
                        'Новая', t.Document_no, t.Zakaz_no, t.Date_needed, t.Date_expected, t.Cost_total_rub
                    FROM `Transactions` t
                    WHERE t.id = v_tx_id;
                END IF;
            END IF;
