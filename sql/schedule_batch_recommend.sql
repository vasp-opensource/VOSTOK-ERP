-- batch_recommend: батч рекомендаций с отдельным логом recommend_log.
...
CREATE TABLE IF NOT EXISTS `recommend_log` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_recommend_log_run` (`run_id`),
    KEY `idx_recommend_log_created_at` (`created_at`),
    KEY `idx_recommend_log_procedure` (`procedure_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP EVENT IF EXISTS ev_batch_recommend_37s;

DROP PROCEDURE IF EXISTS batch_recommend;

DELIMITER $$

CREATE PROCEDURE batch_recommend()
BEGIN
    ...
    BEGIN
        ...
        IF v_batch_lock = 1 THEN
            DO RELEASE_LOCK('batch_recommend');
        END IF;
    ...
    SELECT GET_LOCK('batch_recommend', 0) INTO v_batch_lock;

    IF v_batch_lock = 1 THEN
        SET v_step_no = 1;
        SET v_current_proc = 'recommend_change_purchprod';
        ...
        END IF;
        SET v_step_finished = NOW(6);
        INSERT INTO `recommend_log` (run_id, batch_name, step_no, procedure_name, started_at, finished_at, duration_ms, status, created_at)
        VALUES (v_run_id, 'batch_recommend', v_step_no, v_current_proc, v_step_started, v_step_finished,
                ROUND(TIMESTAMPDIFF(MICROSECOND, v_step_started, v_step_finished) / 1000, 3), 'OK', CURRENT_TIMESTAMP(6));

        SET v_step_no = 2;
        SET v_current_proc = 'recommend_change_wh';
        SET v_step_started = NOW(6);
    ...
    ELSE
        ...
        VALUES (
            ...
            ROUND(TIMESTAMPDIFF(MICROSECOND, v_batch_started, v_batch_finished) / 1000, 3),
            'BLOCKED',
            'Blocked: batch_recommend lock is already held',
            CURRENT_TIMESTAMP(6)
        );
    END IF;
END$$
...
CREATE EVENT ev_batch_recommend_37s
ON SCHEDULE EVERY 37 SECOND
ON COMPLETION PRESERVE
ENABLE
COMMENT 'ERP: recommend batch every 37 seconds'
DO CALL batch_recommend();
