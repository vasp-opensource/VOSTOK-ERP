-- batch_import_check: батч проверки импорта с отдельным логом import_check_log.
-- Интервал запуска: каждые 43 секунды.

CREATE TABLE IF NOT EXISTS `import_check_log` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_import_check_log_run` (`run_id`),
    KEY `idx_import_check_log_created_at` (`created_at`),
    KEY `idx_import_check_log_procedure` (`procedure_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP EVENT IF EXISTS ev_batch_import_check_43s;

DROP PROCEDURE IF EXISTS batch_import_check;

DELIMITER $$

CREATE PROCEDURE batch_import_check()
BEGIN
    ...
    BEGIN
        ...
        IF v_batch_lock = 1 THEN
            DO RELEASE_LOCK('batch_import_check');
        END IF;
    ...
    SELECT GET_LOCK('batch_import_check', 0) INTO v_batch_lock;

    IF v_batch_lock = 1 THEN
        SET v_step_no = 1;
        SET v_current_proc = 'check_imported_quantity';
        ...
        END IF;
        SET v_step_finished = NOW(6);
        INSERT INTO `import_check_log` (run_id, batch_name, step_no, procedure_name, started_at, finished_at, duration_ms, status, created_at)
        VALUES (v_run_id, 'batch_import_check', v_step_no, v_current_proc, v_step_started, v_step_finished,
    ...
    ELSE
        ...
        VALUES (
            ...
            ROUND(TIMESTAMPDIFF(MICROSECOND, v_batch_started, v_batch_finished) / 1000, 3),
            'BLOCKED',
            'Blocked: batch_import_check lock is already held',
            CURRENT_TIMESTAMP(6)
        );
    END IF;
END$$
...
CREATE EVENT ev_batch_import_check_43s
ON SCHEDULE EVERY 43 SECOND
ON COMPLETION PRESERVE
ENABLE
COMMENT 'ERP: import check batch every 43 seconds'
DO CALL batch_import_check();
