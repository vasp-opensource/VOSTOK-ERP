-- batch_prework: предварительные обработчики данных после импорта и до рекомендаций.
-- Запускается из erp_batch_orchestrator как часть основного цикла.

CREATE TABLE IF NOT EXISTS `prework_log` (
    ...
    PRIMARY KEY (`id`),
    KEY `idx_prework_log_run` (`run_id`),
    KEY `idx_prework_log_created_at` (`created_at`),
    KEY `idx_prework_log_procedure` (`procedure_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP PROCEDURE IF EXISTS batch_prework;

DELIMITER $$

CREATE PROCEDURE batch_prework()
BEGIN
    ...
    SELECT GET_LOCK('batch_prework', 0) INTO v_batch_lock;

    IF v_batch_lock = 1 THEN
        SET v_step_no = 1;
        SET v_current_proc = 'prework_address';
        SET v_step_started = NOW(6);
        CALL prework_address();
        ...
        END IF;
        SET v_step_finished = NOW(6);
        INSERT INTO `prework_log` (run_id, batch_name, step_no, procedure_name, started_at, finished_at, duration_ms, status, created_at)
        VALUES (v_run_id, 'batch_prework', v_step_no, v_current_proc, v_step_started, v_step_finished,
                ROUND(TIMESTAMPDIFF(MICROSECOND, v_step_started, v_step_finished) / 1000, 3), 'OK', CURRENT_TIMESTAMP(6));

        SET v_step_no = 2;
        SET v_current_proc = 'prework_default_location';
        SET v_step_started = NOW(6);
        CALL prework_default_location();
        IF @erp_batch_blocked_message IS NOT NULL THEN
            SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO = 3572, MESSAGE_TEXT = @erp_batch_blocked_message;
        END IF;
        SET v_step_finished = NOW(6);
        ...
        SET v_step_no = 3;
        SET v_current_proc = 'prework_location';
        SET v_step_started = NOW(6);
        CALL prework_location();
        IF @erp_batch_blocked_message IS NOT NULL THEN
            SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO = 3572, MESSAGE_TEXT = @erp_batch_blocked_message;
        END IF;
        SET v_step_finished = NOW(6);
        ...
        SET v_step_no = 4;
        SET v_current_proc = 'ch_unite_same_advGroup';
        SET v_step_started = NOW(6);
        CALL ch_unite_same_advGroup();
        IF @erp_batch_blocked_message IS NOT NULL THEN
            SIGNAL SQLSTATE '45000' SET MYSQL_ERRNO = 3572, MESSAGE_TEXT = @erp_batch_blocked_message;
        END IF;
        SET v_step_finished = NOW(6);
    ...
    ELSE
        ...
        VALUES (
            ...
            ROUND(TIMESTAMPDIFF(MICROSECOND, v_batch_started, v_batch_finished) / 1000, 3),
            'BLOCKED',
            'Blocked: batch_prework lock is already held',
            CURRENT_TIMESTAMP(6)
        );
    END IF;
END$$
