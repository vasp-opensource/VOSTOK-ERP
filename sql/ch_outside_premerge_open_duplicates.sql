-- ch_outside_premerge_open_duplicates: объединение «застрявших» дубликатов внешний→склад
-- с одинаковым ERP_ID + Advanced_group (после нормализации TRIM), уже в статусе учёта закупки/изготовления,
-- когда нет входящих строк «Новая»/«Дефицит закупки» — иначе ch_outside_unite их не видит.
-- Вызывать в начале ch_outside_to_purch / ch_outside_to_ownProd (до заполнения tmp_ch_outside_unite_ids).
-- Main не трогаем: сумма по группе не меняется. Новые реквизиты Transactions в суммарной строке — MIN / SUM(Quantity_ordered), как в ch_merge_same_advGroup.
-- Параметр p_kind: 'purch' | 'ownprod'.
--
-- Отбор смягчён относительно первой версии:
-- • Advanced_group: сравнение и группировка по COALESCE(NULLIF(TRIM(Advanced_group), ''), '') — одна группа при отличии только пробелами.
-- • Закупка: Order_prod не обязан быть строго «В закупку» (как в ch_toPurch_inPurchorPayed без этого фильтра).
-- • Status_transaction: допускаются NULL и пустая строка как «открытая» строка вместе с «В ожидании»; исключены Заменено/Отменено.

DELIMITER $$

DROP PROCEDURE IF EXISTS ch_outside_premerge_open_duplicates$$

CREATE PROCEDURE ch_outside_premerge_open_duplicates(IN p_kind VARCHAR(16))
BEGIN
    DECLARE v_merge_left INT DEFAULT 0;
    DECLARE v_erp_id VARCHAR(255);
    DECLARE v_ag_key TEXT;
    DECLARE v_new_id BIGINT UNSIGNED;

    DECLARE v_source VARCHAR(64);
    DECLARE v_wh_merge VARCHAR(64);
    DECLARE v_proc VARCHAR(64);

    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        RESIGNAL;
    END;

    IF p_kind IN ('purch', 'ownprod') THEN
        IF p_kind = 'purch' THEN
            SET v_source          = 'Покупное';
            SET v_wh_merge        = 'В закупке';
            SET v_proc            = 'ch_outside_to_purch';
        ELSE
            SET v_source          = 'Собственное производство';
            SET v_wh_merge        = 'В изготовлении';
            SET v_proc            = 'ch_outside_to_ownProd';
        END IF;

        START TRANSACTION;

            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_ids;
            CREATE TEMPORARY TABLE tmp_ch_premerge_ids (
                `id` INT UNSIGNED PRIMARY KEY,
                `ERP_ID` VARCHAR(255) NOT NULL,
                `Advanced_group` TEXT NULL,
                `Quantity_change` BIGINT NOT NULL
            );

            IF p_kind = 'purch' THEN
                INSERT INTO tmp_ch_premerge_ids (`id`, `ERP_ID`, `Advanced_group`, `Quantity_change`)
                SELECT
                    t.`id`,
                    t.`ERP_ID`,
                    t.`Advanced_group`,
                    COALESCE(t.`Quantity_change`, 0)
                FROM `Transactions` t
                WHERE t.`type` = 'change'
                  AND t.`where_from` = 'внешний'
                  AND t.`where_to` = 'склад'
                  AND t.`Order_purch` IN ('В закупке', 'Оплачено')
                  AND t.`Status_warehouse` = 'В закупке'
                  AND COALESCE(t.`Quantity_change`, 0) <> 0
                  AND (t.`created_by` IS NULL OR t.`created_by` <> 'ch_outside_to_purch')
                  AND (t.`Status_transaction` IS NULL
                       OR TRIM(t.`Status_transaction`) = ''
                       OR t.`Status_transaction` = 'В ожидании')
                  AND (t.`Status_transaction` IS NULL OR t.`Status_transaction` NOT IN ('Заменено', 'Отменено'))
                  AND EXISTS (
                      SELECT 1
                      FROM `Transactions` t2
                      WHERE COALESCE(NULLIF(TRIM(t2.`Advanced_group`), ''), '')
                          = COALESCE(NULLIF(TRIM(t.`Advanced_group`), ''), '')
                        AND t2.`ERP_ID` = t.`ERP_ID`
                        AND t2.`id` <> t.`id`
                        AND t2.`type` = 'change'
                        AND t2.`where_from` = 'внешний'
                        AND t2.`where_to` = 'склад'
                        AND t2.`Order_purch` IN ('В закупке', 'Оплачено')
                        AND t2.`Status_warehouse` = 'В закупке'
                        AND COALESCE(t2.`Quantity_change`, 0) <> 0
                        AND (t2.`created_by` IS NULL OR t2.`created_by` <> 'ch_outside_to_purch')
                        AND (t2.`Status_transaction` IS NULL
                             OR TRIM(t2.`Status_transaction`) = ''
                             OR t2.`Status_transaction` = 'В ожидании')
                        AND (t2.`Status_transaction` IS NULL OR t2.`Status_transaction` NOT IN ('Заменено', 'Отменено'))
                  );
            ELSE
                INSERT INTO tmp_ch_premerge_ids (`id`, `ERP_ID`, `Advanced_group`, `Quantity_change`)
                SELECT
                    t.`id`,
                    t.`ERP_ID`,
                    t.`Advanced_group`,
                    COALESCE(t.`Quantity_change`, 0)
                FROM `Transactions` t
                WHERE t.`type` = 'change'
                  AND t.`where_from` = 'внешний'
                  AND t.`where_to` = 'склад'
                  AND t.`Order_purch` = 'Собственное производство'
                  AND t.`Order_prod` = 'Принято в изготовление'
                  AND t.`Status_warehouse` = 'В изготовлении'
                  AND COALESCE(t.`Quantity_change`, 0) <> 0
                  AND (t.`created_by` IS NULL OR t.`created_by` <> 'ch_outside_to_ownProd')
                  AND (t.`Status_transaction` IS NULL
                       OR TRIM(t.`Status_transaction`) = ''
                       OR t.`Status_transaction` = 'В ожидании')
                  AND (t.`Status_transaction` IS NULL OR t.`Status_transaction` NOT IN ('Заменено', 'Отменено'))
                  AND EXISTS (
                      SELECT 1
                      FROM `Transactions` t2
                      WHERE COALESCE(NULLIF(TRIM(t2.`Advanced_group`), ''), '')
                          = COALESCE(NULLIF(TRIM(t.`Advanced_group`), ''), '')
                        AND t2.`ERP_ID` = t.`ERP_ID`
                        AND t2.`id` <> t.`id`
                        AND t2.`type` = 'change'
                        AND t2.`where_from` = 'внешний'
                        AND t2.`where_to` = 'склад'
                        AND t2.`Order_purch` = 'Собственное производство'
                        AND t2.`Order_prod` = 'Принято в изготовление'
                        AND t2.`Status_warehouse` = 'В изготовлении'
                        AND COALESCE(t2.`Quantity_change`, 0) <> 0
                        AND (t2.`created_by` IS NULL OR t2.`created_by` <> 'ch_outside_to_ownProd')
                        AND (t2.`Status_transaction` IS NULL
                             OR TRIM(t2.`Status_transaction`) = ''
                             OR t2.`Status_transaction` = 'В ожидании')
                        AND (t2.`Status_transaction` IS NULL OR t2.`Status_transaction` NOT IN ('Заменено', 'Отменено'))
                  );
            END IF;

            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_agg;
            CREATE TEMPORARY TABLE tmp_ch_premerge_agg AS
            SELECT
                x.`ERP_ID` AS `ERP_ID`,
                COALESCE(NULLIF(TRIM(x.`Advanced_group`), ''), '') AS `ag_key`,
                COUNT(*) AS `cnt`,
                SUM(COALESCE(t.`Quantity_change`, 0)) AS `sum_qty`
            FROM tmp_ch_premerge_ids x
            INNER JOIN `Transactions` t ON t.`id` = x.`id`
            GROUP BY x.`ERP_ID`, COALESCE(NULLIF(TRIM(x.`Advanced_group`), ''), '')
            HAVING `cnt` >= 2 AND SUM(COALESCE(t.`Quantity_change`, 0)) <> 0;

            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_replace_queue;
            CREATE TEMPORARY TABLE tmp_ch_premerge_replace_queue (
                `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
                `ERP_ID` VARCHAR(255) NOT NULL,
                `ag_key` TEXT NOT NULL
            );

            INSERT INTO tmp_ch_premerge_replace_queue (`ERP_ID`, `ag_key`)
            SELECT `ERP_ID`, `ag_key` FROM tmp_ch_premerge_agg;

            replace_loop: LOOP
                SELECT COUNT(*) INTO v_merge_left FROM tmp_ch_premerge_replace_queue;
                IF v_merge_left = 0 THEN
                    LEAVE replace_loop;
                END IF;

                SELECT `ERP_ID`, `ag_key` INTO v_erp_id, v_ag_key
                FROM tmp_ch_premerge_replace_queue
                ORDER BY `id`
                LIMIT 1;

                INSERT INTO `Transactions` (
                    ERP_ID, linked_transaction, type, where_from, where_to,
                    Quantity_of_parts_total, Quantity_change, Status_transaction,
                    Project, Target_assembly, Supplied_component_number, Component_revision, Component_name,
                    Quantity_in_target_assembly, Quantity_of_target_assemblies, Components_quantity_in_assembly,
                    Component_type, For_supplied_as_assembly_components_provided_by_supplier, Part_material,
                    Producer, Catalogue_number, Producer_article, Distributer, Distributer_article,
                    MBOM_type, Mass_kg, Unit_of_measure, Height, Width, Length,
                    Advanced_group, Address,
                    Recommend_purchprod,
                    Order_purch, Order_wh, Order_prod, Order_OTK,
                    Order_sv, Recommend_wh, Quantity_ordered, Replace_to, Rework_to, Rework_from,
                    Status_warehouse,
                    Document_no, Document_date, Zakaz_no, Date_needed, Date_expected, Cost_total_rub,
                    Supplier, Location, Source, Initial_doc_no,
                    created_by, updated_by, created_at, updated_at
                )
                SELECT
                    g.`ERP_ID`,
                    NULL,
                    'change',
                    'внешний',
                    'склад',
                    0,
                    g.`sum_qty`,
                    CASE WHEN g.`sum_qty` <= 0 THEN 'Отменено' ELSE 'В ожидании' END,
                    fld.`Project`, fld.`Target_assembly`, fld.`Supplied_component_number`, fld.`Component_revision`, fld.`Component_name`,
                    fld.`Quantity_in_target_assembly`, fld.`Quantity_of_target_assemblies`, fld.`Components_quantity_in_assembly`,
                    fld.`Component_type`, fld.`For_supplied_as_assembly_components_provided_by_supplier`, fld.`Part_material`,
                    fld.`Producer`, fld.`Catalogue_number`, fld.`Producer_article`, fld.`Distributer`, fld.`Distributer_article`,
                    fld.`MBOM_type`, fld.`Mass_kg`, fld.`Unit_of_measure`, fld.`Height`, fld.`Width`, fld.`Length`,
                    fld.`Advanced_group`, fld.`Address`,
                    fld.`Recommend_purchprod`,
                    fld.`Order_purch`,
                    NULL,
                    fld.`Order_prod`,
                    fld.`Order_OTK`,
                    fld.`Order_sv`,
                    fld.`Recommend_wh`,
                    fld.`sum_qty_ord`,
                    fld.`Replace_to`,
                    fld.`Rework_to`,
                    fld.`Rework_from`,
                    CASE WHEN g.`sum_qty` <= 0 THEN 'Норма' ELSE v_wh_merge END,
                    fld.`Document_no`,
                    fld.`Document_date`,
                    fld.`Zakaz_no`, fld.`Date_needed`, fld.`Date_expected`, fld.`Cost_total_rub`,
                    fld.`Supplier`, fld.`Location`, fld.`Source`, fld.`Initial_doc_no`,
                    v_proc,
                    v_proc,
                    NOW(),
                    NOW()
                FROM tmp_ch_premerge_agg g
                INNER JOIN (
                    SELECT
                        t.`ERP_ID`,
                        COALESCE(NULLIF(TRIM(t.`Advanced_group`), ''), '') AS `ag_key`,
                        MIN(t.`Project`) AS `Project`,
                        MIN(t.`Target_assembly`) AS `Target_assembly`,
                        MIN(t.`Supplied_component_number`) AS `Supplied_component_number`,
                        MIN(t.`Component_revision`) AS `Component_revision`,
                        MIN(t.`Component_name`) AS `Component_name`,
                        MIN(t.`Quantity_in_target_assembly`) AS `Quantity_in_target_assembly`,
                        MIN(t.`Quantity_of_target_assemblies`) AS `Quantity_of_target_assemblies`,
                        MIN(t.`Components_quantity_in_assembly`) AS `Components_quantity_in_assembly`,
                        MIN(t.`Component_type`) AS `Component_type`,
                        MIN(t.`For_supplied_as_assembly_components_provided_by_supplier`) AS `For_supplied_as_assembly_components_provided_by_supplier`,
                        MIN(t.`Part_material`) AS `Part_material`,
                        MIN(t.`Producer`) AS `Producer`,
                        MIN(t.`Catalogue_number`) AS `Catalogue_number`,
                        MIN(t.`Producer_article`) AS `Producer_article`,
                        MIN(t.`Distributer`) AS `Distributer`,
                        MIN(t.`Distributer_article`) AS `Distributer_article`,
                        MIN(t.`MBOM_type`) AS `MBOM_type`,
                        MIN(t.`Mass_kg`) AS `Mass_kg`,
                        MIN(t.`Unit_of_measure`) AS `Unit_of_measure`,
                        MIN(t.`Height`) AS `Height`,
                        MIN(t.`Width`) AS `Width`,
                        MIN(t.`Length`) AS `Length`,
                        MIN(t.`Advanced_group`) AS `Advanced_group`,
                        MIN(t.`Address`) AS `Address`,
                        MIN(t.`Recommend_purchprod`) AS `Recommend_purchprod`,
                        MIN(t.`Document_no`) AS `Document_no`,
                        MIN(t.`Document_date`) AS `Document_date`,
                        MIN(t.`Zakaz_no`) AS `Zakaz_no`,
                        MIN(t.`Date_needed`) AS `Date_needed`,
                        MIN(t.`Date_expected`) AS `Date_expected`,
                        MIN(t.`Cost_total_rub`) AS `Cost_total_rub`,
                        MIN(t.`Supplier`) AS `Supplier`,
                        MIN(t.`Location`) AS `Location`,
                        MIN(t.`Source`) AS `Source`,
                        MIN(t.`Initial_doc_no`) AS `Initial_doc_no`,
                        MIN(t.`Order_purch`) AS `Order_purch`,
                        MIN(t.`Order_prod`) AS `Order_prod`,
                        MIN(t.`Order_OTK`) AS `Order_OTK`,
                        MIN(t.`Order_sv`) AS `Order_sv`,
                        MIN(t.`Recommend_wh`) AS `Recommend_wh`,
                        SUM(COALESCE(t.`Quantity_ordered`, 0)) AS `sum_qty_ord`,
                        MIN(t.`Replace_to`) AS `Replace_to`,
                        MIN(t.`Rework_to`) AS `Rework_to`,
                        MIN(t.`Rework_from`) AS `Rework_from`
                    FROM `Transactions` t
                    INNER JOIN tmp_ch_premerge_ids x ON x.`id` = t.`id`
                    GROUP BY t.`ERP_ID`, COALESCE(NULLIF(TRIM(t.`Advanced_group`), ''), '')
                ) fld
                  ON fld.`ERP_ID` = g.`ERP_ID`
                 AND fld.`ag_key` = g.`ag_key`
                WHERE g.`cnt` >= 2
                  AND g.`ERP_ID` = v_erp_id
                  AND g.`ag_key` = v_ag_key;

                SET v_new_id = LAST_INSERT_ID();

                UPDATE `Transactions` t
                SET
                    t.`linked_transaction` = v_new_id,
                    t.`Source`             = v_source,
                    t.`updated_at`         = NOW(),
                    t.`updated_by`         = CASE
                                                WHEN t.`updated_by` IS NULL OR TRIM(COALESCE(t.`updated_by`, '')) = '' THEN v_proc
                                                ELSE CONCAT(t.`updated_by`, '; ', v_proc)
                                             END
                WHERE t.`id` = v_new_id;

                UPDATE `Transactions` t
                INNER JOIN tmp_ch_premerge_ids x ON x.`id` = t.`id`
                INNER JOIN tmp_ch_premerge_agg g
                  ON g.`ERP_ID` = x.`ERP_ID`
                 AND g.`ag_key` = COALESCE(NULLIF(TRIM(x.`Advanced_group`), ''), '')
                SET
                    t.`linked_transaction` = v_new_id,
                    t.`created_by`         = v_proc,
                    t.`Status_transaction` = 'Заменено',
                    t.`Status_warehouse` = 'Норма',
                    t.`Source`             = v_source,
                    t.`updated_at`         = NOW(),
                    t.`updated_by`         = CASE
                                                WHEN t.`updated_by` IS NULL OR TRIM(COALESCE(t.`updated_by`, '')) = '' THEN v_proc
                                                ELSE CONCAT(t.`updated_by`, '; ', v_proc)
                                             END
                WHERE g.`cnt` >= 2
                  AND g.`ERP_ID` = v_erp_id
                  AND g.`ag_key` = v_ag_key;

                DELETE FROM tmp_ch_premerge_replace_queue
                WHERE `ERP_ID` = v_erp_id AND `ag_key` <=> v_ag_key;
            END LOOP replace_loop;

            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_replace_queue;
            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_agg;
            DROP TEMPORARY TABLE IF EXISTS tmp_ch_premerge_ids;

        COMMIT;
    END IF;
END$$

DELIMITER ;

/*
Диагностика в БД (вручную): кандидаты «В закупке» с дубликатом ag_norm по ERP_ID:

SELECT
    t.id,
    t.ERP_ID,
    t.Advanced_group,
    COALESCE(NULLIF(TRIM(t.Advanced_group), ''), '') AS ag_norm,
    t.Order_purch,
    t.Order_prod,
    t.Status_warehouse,
    t.Status_transaction,
    t.Quantity_change,
    t.created_by
FROM `Transactions` t
WHERE t.`type` = 'change'
  AND t.`where_from` = 'внешний'
  AND t.`where_to` = 'склад'
  AND t.`Order_purch` IN ('В закупке', 'Оплачено')
  AND t.`Status_warehouse` = 'В закупке'
  AND COALESCE(t.`Quantity_change`, 0) <> 0
ORDER BY t.`ERP_ID`, ag_norm, t.`id`;
*/
